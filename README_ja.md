...
リポジトリ: <https://github.com/masayukitanaka/manga-composer>
...
> **注意:** manga-composer はまだ npm レジストリに公開していません。GitHub から
> 直接インストールしてください。（`prepare` スクリプトによりインストール時に
> 自動でビルドされるため、手動のビルド手順は不要です。）
...
特定のブランチ・タグ・コミットを指定することもできます:

```bash
npm install github:masayukitanaka/manga-composer#main
```

またはクローンして npm link で開発用に使う:
...
npm install
npm run build
npm link            # `manga-composer` コマンドをグローバルで使えるようにする
```
