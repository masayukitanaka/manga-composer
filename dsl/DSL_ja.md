...
| `flip_h` | `true` \| `false` | `false` | 画像を左右反転（左右ミラー）して描画するか |
...
| 属性 | 型 | デフォルト | 説明 |
|---|---|---|---|
| `text` | string | なし（必須） | 表示テキスト。インラインの `<i>…</i>` / `<b>…</b>` に対応（下記「フォント・組版」参照） |
| `text_direction` | `horizontal` \| `vertical` | `horizontal` | 文字方向 |
| `font_size` | number (mm) | `4.5` | フォントサイズ |
| `font_family` | string | （日本語優先の sans-serif スタック） | CSS風のカンマ区切り候補（優先順）。例 `"Yu Gothic, Hiragino Sans, sans-serif"`。利用可能な最初のものが使われる。環境非依存にしたい場合はPNGレンダラにフォント実体を渡す |
| `line_height` | number \| `<n>mm` | `1.4` | 行送り（行間）。無単位・`%` は `font_size` に対する倍率（`1.8`・`150%`）、`mm` は絶対行送り |
| `letter_spacing` | number (mm) | `0` | 字送り（字間）の追加アキ（負値可） |
| `font_style` | `normal` \| `italic` | `normal` | 要素全体の斜体（インライン `<i>` が優先） |
| `font_weight` | `normal` \| `bold` | `normal` | 要素全体の太字（インライン `<b>` が優先） |
| `wrap` | `true` \| `false` | `true` | 自動折り返し。`false` で無効化——テキストは明示的な `\n` でのみ改行し、はみ出しを許容する（作者が改行位置を完全に制御できる） |
...
- `font_family` / `line_height` / `letter_spacing` は要素全体に効きます。`font_style` / `font_weight` で要素全体の斜体・太字を指定します。
- 部分的な強調にはインラインタグを使います: `<i>…</i>`（斜体）・`<b>…</b>`（太字）。大文字小文字は区別せず、入れ子も可能です。文字としての `<` は `\<` と書きます。

```manga
balloon {
  text: "それは<b>絶対に</b><i>やらない</i>方針だ。"
...
- **`wrap: false`** で自動折り返しを無効化します。テキストは文字列中の改行（`\n`）の位置でのみ改行され、箱からはみ出すのを許容します——1行キャプションなど、自動折り返しが変な位置で改行してしまう場合に有効です。`wrap: true`（デフォルト）ではテキストが箱に収まるよう折り返されます（スペース区切りは単語境界、CJKは任意位置）。

```manga
monologue {
  // 改行の位置でのみ改行。各行は箱の端を越えてはみ出しうる。
