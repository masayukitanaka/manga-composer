 ...
 */
export function defaultTransposeOptions() {
    return {
        direction: "flip",
        text: false,
        flipImages: true,
 ...
 */
function mirrorTailAngle(deg) {
    const m = (180 - deg) % 360;
    return m < 0 ? m + 360 : m;
}
// ── Page ────────────────────────────────────────────────────────────────────
function transposeConfig(cfg, opts) {
    const next = { ...cfg };
    // direction
    if (opts.direction === "flip")
        next.direction = flipDirection(cfg.direction);
    else
        next.direction = opts.direction; // force target (idempotent handled by caller)
...
export function directionChanges(cfg, opts) {
    if (opts.direction === "flip")
        return true;
    return cfg.direction !== opts.direction;
}
// ── row / col ───────────────────────────────────────────────────────────────
function transposeContainer(node, ctx) {
    const { opts } = ctx;
    const next = { ...node };
    if (opts.align)
        next.align = mirrorAlign(node.align);
    // margin left⇄right
...
function negNullable(v) {
    return v === null ? null : -v;
}
// ── panel ─────────────────────────────────────────────────────────────────
function transposePanel(node, ctx) {
    const { opts } = ctx;
    const a = { ...node.attrs };
    // border left⇄right
...
// ── image layer ─────────────────────────────────────────────────────────────
function transposeImageLayer(layer, ctx, panelId) {
    const { opts } = ctx;
    const next = { ...layer };
    if (opts.align)
        next.anchorPos = mirrorAnchorPos(layer.anchorPos);
    if (opts.flipImages)
        next.flipH = !layer.flipH;
    ...
    if (!opts.keepCoords && layer.x !== null) {
        ...
        if (layer.x.unit === "%") {
            ...
            ctx.warn(`panel "${panelId}": image layer x is in ${layer.x.unit} (panel-relative); ` +
...
function negLength(l) {
    return { value: -l.value, unit: l.unit };
}
// ── balloon / monologue ─────────────────────────────────────────────────────
function transposeSpeech(node, ctx) {
    const { opts } = ctx;
    const a = { ...node.attrs };
    if (opts.align) {
    ...
    if (!opts.keepCoords && node.attrs.x !== null) {
        if (node.attrs.width !== null) {
            ...
            ctx.warn(`${node.kind}${node.id ? ` "${node.id}"` : ""}: absolute x with auto width ` +
                `can't be mirrored without layout — kept as-is. Set an explicit width, or --keep-coords.`);
        }
    }
    if (node.kind === "balloon" && opts.tail) {
        const b = a;
        b.tailAngle = mirrorTailAngle(node.attrs.tailAngle);
    }
    return { ...node, attrs: a };
}
function transposeNode(node, ctx) {
    switch (node.kind) {
        case "panel":
 ...
 */
export function transposePage(page, opts) {
    const warnings = [];
    const ctx = {
        opts,
