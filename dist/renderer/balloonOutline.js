 ...
 */
function _balloon_geometry(speech) {
    const r = speech.rect;
...
function _balloon_outline(speech, g) {
    const attrs = speech.attrs;
    const { cx, cy, rx, ry, tail_edge, tail_pos, seed } = g;
    if (attrs.shape === "oval" || attrs.shape === "whisper" || attrs.shape === "thought") {
        const [points, tail_index] = _ellipse_outline_points(cx, cy, rx, ry, tail_edge, tail_pos, seed, 16, attrs.jitter);
        return { points, tail_index, sharp: [] };
    }
    if (attrs.shape === "shout" || attrs.shape === "jagged" || attrs.shape === "explosion") {
        const [points, tail_index, sharp] = _star_outline_points(cx, cy, rx, ry, attrs.shape, tail_edge, tail_pos, seed, attrs.innerRatio, attrs.jitter);
        return { points, tail_index, sharp };
    }
    ...
    const [points, tail_index, sharp] = _rounded_rect_outline_points(cx, cy, rx, ry, attrs.cornerRadius, tail_angle);
    return { points, tail_index, sharp };
}
/**
 ...
 */
export function resolveBalloonTail(speech) {
...
function _tail_tip(root, cx, cy, length) {
    const dx = root[0] - cx;
    const dy = root[1] - cy;
...
export function renderBalloon(renderer, parent, speech) {
    const r = speech.rect;
    const attrs = speech.attrs;
...
function _edge_pos_to_angle(edge, pos_pct) {
    ...
    if (edge === "right")
        ...
        angle = (3 * Math.PI) / 4 + t * (Math.PI / 2);
    else
        angle = (-3 * Math.PI) / 4 + t * (Math.PI / 2);
    const two_pi = 2 * Math.PI;
    return ((angle % two_pi) + two_pi) % two_pi;
}
function _angle_between(a, a0, a1) {
    const two_pi = 2 * Math.PI;
...
// ── outline generators ──────────────────────────────────────────────────────
function _ellipse_outline_points(cx, cy, rx, ry, edge, pos_pct, seed, n = 16, jitter_scale = 1.0) {
...
function _star_outline_points(cx, cy, rx, ry, shape, edge, pos_pct, seed, inner_ratio = null, jitter_scale = 1.0) {
    const jitter = new SeededJitter(seed);
    const n_points = 14;
    let inner_center;
    if (inner_ratio !== null) {
    ...
    for (let i = 0; i < angles.length; i++) {
        ...
        if (outer && mark_outer_sharp)
            sharp_indices.push(points.length);
        points.push([px, py]);
    }
    if (tail_angle !== null) {
 ...
 */
function _rounded_rect_outline_points(cx, cy, rx, ry, corner_radius, tail_angle) {
    const x0 = cx - rx;
    const y0 = cy - ry;
    const x1 = cx + rx;
    const y1 = cy + ry;
    ...
    const arc = (center, start_angle, end_angle) => {
        const pts = [];
 ...
 */
function _ray_exit_on_rect(cx, cy, x0, y0, x1, y1, angle) {
 ...
 */
function _find_segment_containing(points, p) {
...
function _insert_tail_notch(points, tail_index, tip, existing_sharp = null) {
    if (tail_index === null) {
    ...
    const uy = ty / tlen;
    const half = 2.1;
    const root_left = [root[0] - ux * half, root[1] - uy * half];
    const root_right = [root[0] + ux * half, root[1] + uy * half];
    const new_points = [
    ...
    const merged = new Set([...notch_sharp, ...reindexed]);
    return [new_points, [...merged].sort((p, q) => p - q)];
}
function _points_to_smooth_path_d(points, sharp_indices = null) {
    const n = points.length;
    if (n < 3)
        return "";
    const sharp = new Set(sharp_indices ?? []);
    const catmull_rom_to_bezier = (p0, p1, p2, p3) => {
...
//# sourceMappingURL=balloonOutline.js.map