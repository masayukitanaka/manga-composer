...
import { transposePage, defaultTransposeOptions } from "./transpose.js";
 ...
 */
function renderPageToFile(page, sourceDir, outputPath, fmt, opts) {
...
program
    ...
    try {
        ...
        process.stdout.write(`Layouted ${panels.length} panel(s), ${speeches.length} speech element(s)\n`);
...
program
    ...
    .option("--font-dir <path>", "Font directory for --out-image PNG rendering (repeatable)", (val, acc) => [...acc, val], [])
    .option("--font-file <path>", "Font file for --out-image PNG rendering (repeatable)", (val, acc) => [...acc, val], [])
    .action((input, opts) => {
    try {
        ...
        if (opts.dryRun) {
            process.stderr.write(`Dry run: direction ${page.config.direction} → ${transposed.config.direction}` +
        ...
        if (opts.outImage) {
            ...
            if (fmt === "auto") {
                ...
                if (ext === ".svg")
                    ...
                    process.stderr.write(`Error: cannot infer image format from ${opts.outImage}; use --image-format png|svg\n`);
                    process.exit(1);
                }
            }
            if (fmt !== "png" && fmt !== "svg") {
