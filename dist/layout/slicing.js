...
export class SkewLine {
    ...
    mid_y;
    skew_angle;
    constructor(base_x, // X of this panel's own boundary at mid_y
    mid_y, // Shared reference Y (midpoint of the taller panel)
    skew_angle) {
        this.base_x = base_x;
        this.mid_y = mid_y;
...
export class SkewHLine {
    ...
    mid_x;
    skew_angle;
    constructor(base_y, // Y of the gutter centre at mid_x
    mid_x, // X reference (midpoint of panel width)
    skew_angle) {
        this.base_y = base_y;
        this.mid_x = mid_x;
        this.skew_angle = skew_angle;
...
export class LayoutedSpeech {
    ...
    attrs;
    has_tail;
    constructor(kind, // "balloon" | "monologue"
    id, rect, attrs, has_tail = false) {
        this.kind = kind;
        this.id = id;
        this.rect = rect;
        this.attrs = attrs;
 ...
 */
export function resolveImageLayerRect(layer, r) {
    const wLen = (l, fallback) => l === null ? fallback : l.unit === "%" ? (l.value * r.w) / 100 : l.value;
    const hLen = (l, fallback) => l === null ? fallback : l.unit === "%" ? (l.value * r.h) / 100 : l.value;
    ...
    if (layer.x !== null)
        x = r.x + (layer.x.unit === "%" ? (layer.x.value * r.w) / 100 : layer.x.value);
    if (layer.y !== null)
        y = r.y + (layer.y.unit === "%" ? (layer.y.value * r.h) / 100 : layer.y.value);
...
export function isFullWidthChar(ch) {
...
export function charAdvance(ch, font_size, letter_spacing = 0.0) {
...
export function measureTextWidth(text, font_size, letter_spacing = 0.0) {
    let w = 0;
    for (const ch of text)
        w += charAdvance(ch, font_size, letter_spacing);
    return w;
}
/**
 ...
 */
export function resolveLineHeight(lineHeight, font_size) {
    if (lineHeight.unit === "mm")
        return lineHeight.value;
    return font_size * lineHeight.value; // "%"/unitless multiplier
}
/** Rough width/height estimate (mm) for a text block. */
export function _estimate_text_box_size(text, font_size, direction, lineHeight = { value: TEXT_LINE_H_FACTOR, unit: "%" }, letterSpacing = 0.0, wrap = true) {
    ...
    if (direction === "vertical") {
        ...
        for (const para of plain.split("\n")) {
            if (!para.includes(" "))
                continue;
            for (const word of para.split(" "))
                longest_token = Math.max(longest_token, word.length);
        }
...
export class LayoutEngine {
    ...
    layout() {
        ...
        const inner = new Rect(pad_left, pad_top, cfg.widthMm - pad_left - pad_right, cfg.heightMm - pad_top - pad_bottom);
    ...
    _layout_children(nodes, bounds, axis, gutter) {
        if (nodes.length === 0)
            return;
    ...
    _compute_sizes(nodes, bounds, axis, gutter) {
        ...
        if (remaining < 0) {
            throw new LayoutError(`Size specifications exceed available space: ` +
    ...
    _layout_node(node, rect) {
        if (node.kind === "panel") {
            ...
            const inner = new Rect(rect.x + node.marginLeft, rect.y + node.marginTop, rect.w - node.marginLeft - node.marginRight, rect.h - node.marginTop - node.marginBottom);
    ...
    _adjust_skewline_y_for_slanted_top() {
        const _intersect = (hsl, vsl) => {
    ...
    _unify_skewline_mid_y() {
        const right_groups = new Map();
    ...
    _link_tb(top, bottom, rt, rb, gap) {
        ...
        const EPSILON2 = 0.01;
        const top_covers_bottom = rt.x <= rb.x + EPSILON2 && rt.x + rt.w >= rb.x + rb.w - EPSILON2;
    ...
    _resolve_speech_elements() {
        for (const owner_panel of this.panels) {
            for (const node of owner_panel.speeches) {
                ...
                const aspect_ratio = node.kind === "balloon" ? node.attrs.aspectRatio : null;
                if (width !== null && height === null && aspect_ratio) {
                    ...
                    const nlines = countWrappedLines(plainText(attrs.text), max_w, attrs.fontSize, attrs.letterSpacing);
                    const line_h = resolveLineHeight(attrs.lineHeight, attrs.fontSize);
                    height = nlines * line_h + pad2;
                }
                else if (width === null || height === null) {
                    const [est_w, est_h] = _estimate_text_box_size(attrs.text, attrs.fontSize, attrs.textDirection, attrs.lineHeight, attrs.letterSpacing, attrs.wrap);
                    const pad2 = attrs.padding * 2;
                ...
                this.speeches.push(new LayoutedSpeech(kind, node.id, rect, attrs, kind === "balloon"));
            }
        }
    }
}
