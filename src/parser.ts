...
// ── unified length normalization ────────────────────────────────────────────
//
// Three DSL contexts turn a value into a {value, unit} Length: row/col sizes,
// image-layer geometry, and line_height. They share the same
// normalize → validate-unit → validate-positive shape but differ in two
// deliberate ways, captured by LengthPolicy:
//   - `bareNumber`: what a unit-less number means ("mm" for geometry,
//     "multiplier" for line_height, "reject" for row/col which require a unit).
//   - `allowedUnits`: which units survive validation (row/col accept px/pt;
//     the others only mm/%).
// `percentDivisor` handles line_height's `140%` → 1.4 folding (divisor 100);
// the others keep the percent number as-is (divisor 1).
interface LengthPolicy {
  /** Human field name for error messages (e.g. "line_height", "image layer x"). */
  field: string;
  /** Meaning of a unit-less number. */
  bareNumber: "mm" | "multiplier" | "reject";
  /** Units that pass validation. */
  allowedUnits: readonly Unit[];
  /** Divide a `%` number by this before storing (100 for line_height, else 1). */
 ...
 */
function normalizeLength(val: RawValue, policy: LengthPolicy): Length {
  ...
  if (!policy.allowedUnits.includes(len.unit)) {
    throw new ParseError(
      `${policy.field} must use ${policy.allowedUnits.join("/")} (got ${JSON.stringify(len.unit)})`,
    );
  }
...
function toBool(text: string, field: string): boolean {
...
class Parser {
  ...
  private seenPanelIds = new Set<string>();
  ...
  private parseLengthValue(field = "size"): Length {
  ...
  private parseOneContainerAttr(node: RowNode | ColNode, kind: "row" | "col"): void {
    ...
    if (key === "height") {
      ...
      (node as RowNode).height = this.parseLengthValue("row height");
      return;
    }
    if (key === "width") {
      if (kind !== "col") throw new ParseError(`'width' is only valid on col`);
      (node as ColNode).width = this.parseLengthValue("col width");
      return;
    }
    if (key === "gutter") {
  ...
  private parsePanelStmt(): PanelNode {
    ...
    let imagesBlock: ImageLayer[] | null = null;
   ...
   */
  private parsePanelDefBlock(): [Record<string, RawValue>, SpeechNode[], ImageLayer[] | null] {
    this.expect("LBRACE");
   ...
   */
  private parseImagesBlock(): ImageLayer[] {
  ...
  private parseImageLayer(): ImageLayer {
    this.expect("LBRACE");
    ...
    if ("image_fit" in raw) {
      ...
      layer.imageFit = assertLiteral<ImageFit>(v as string, IMAGE_FITS, "image_fit");
    }
    if ("anchor_pos" in raw) {
      const v = raw.anchor_pos.kind === "scalar" ? raw.anchor_pos.text : "";
      layer.anchorPos = assertLiteral<AnchorPos>(v, ANCHOR_POSITIONS, "anchor_pos");
    }
    if ("x" in raw) layer.x = this.imageLayerLength(raw.x, "x");
    if ("y" in raw) layer.y = this.imageLayerLength(raw.y, "y");
    if ("width" in raw) layer.width = this.imageLayerLength(raw.width, "width", true);
    if ("height" in raw) layer.height = this.imageLayerLength(raw.height, "height", true);
    if ("dx" in raw) layer.dx = this.imageLayerLength(raw.dx, "dx");
    if ("dy" in raw) layer.dy = this.imageLayerLength(raw.dy, "dy");
    if ("clip" in raw) {
   ...
   */
  private imageLayerLength(val: RawValue, field: string, positive = false): Length {
   ...
   */
  private resolveImageLayers(
    raw: Record<string, RawValue>,
    attrs: PanelAttrs,
    imagesBlock: ImageLayer[] | null,
  ): ImageLayer[] {
    ...
    if (imagesBlock !== null) return imagesBlock;
    ...
    for (const [name, val] of Object.entries(raw)) {
      ...
      switch (name) {
        ...
        case "image_fit":
          ...
          break;
        case "image_clip":
          attrs.imageClip = toBool(v as string, "image_clip");
          break;
        case "label":
          attrs.label = v as string;
          break;
        case "description":
          attrs.description = v as string;
          break;
        case "show_description":
          attrs.showDescription = toBool(v as string, "show_description");
          break;
        case "text":
