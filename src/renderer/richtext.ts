 ...
 */
export function parseRichText(text: string, base: { italic: boolean; bold: boolean }): TextRun[] {
  const runs: TextRun[] = [];
  const stack: Tag[] = []; // open tags, innermost last
  let buf = "";
    ...
    if (ch === "<") {
      ...
      if (!m) {
        throw new ParseError(
          `Invalid inline tag in text near "${text.slice(i, i + 8)}" (use \\< for a literal '<')`,
        );
      }
      const closing = m[1] === "/";
