...
See the [`dsl/`](dsl/) directory ([`DSL.md`](dsl/DSL.md),
[`DSL_ja.md`](dsl/DSL_ja.md)) for the full DSL reference.
...
Tests are written with [Vitest](https://vitest.dev/) and live in `test/unit/`.

```bash
npm test                       # run the whole suite once (vitest run)
npm run test:watch             # re-run on change (interactive watch mode)
...
# update inline/file snapshots after an intentional output change
npx vitest run -u
```

What the suite covers:

- **Unit** — lexer, parser, layout engine, serializer round-trip, text/font
  metrics, balloon geometry, skew corner cases.
- **Golden SVG** (`compile.test.ts`) — compiles the bundled `examples/` and
  `examples2/` `.manga` files and structurally diffs the output against the
  reference SVGs in `test/references/` (element tree + numeric attributes within
  a small tolerance). Balloon outlines are compared at bounding-box level, since
  their hand-drawn jitter uses an internally-deterministic PRNG whose exact
  wobble differs from the original Python `manga-gen`.

The reference SVGs are bundled in the repo, so the suite is self-contained —
running `npm test` needs nothing beyond the dev dependencies. Some `examples/`
files exercise TS-only features (image layers, rich text) and have no reference
SVG; those are skipped by the golden diff but still covered by the other tests.
